...
class Register(CreateView):
	...
	def form_valid(self, form):
		...
		message = render_to_string('registration/acctive_email.html', {
			'user': user,
...
def activate(request, uidb64, token):
	...
	if user is not None and account_activation_token.check_token(user, token):
		user.is_active = True
		user.save()
		return HttpResponse('حساب کاربری شما با موفقیت فعال شد برای ورود <a href="/login" >کلیک</a> کنید')
	else:
		return HttpResponse('این لینک منقضی شده است، برای فعال سازی مجدد اکانت خود <a href="/register" >کلیک</a> کنید ')		
